...
sd.initPara = function(para){
  ...
  //广告回传依赖$url，所有事件都要有$url
  if(sd.para.preset_properties.latest_wx_ad_click_id){
    sd.para.preset_properties.url = true;
  }
  ...
  checkIsFirstLatest: function() {
    ...
    _.each(sd.para.preset_properties,function(value,key){
      ...
      if(value){
        ...
        }else if(key == 'wx_ad_click_id' && sd.store._state.props){
...
sd.init = function(para){
  ...
  // iOS Safari
  if (sd._.isIOS() && sd._.getIOSVersion() && sd._.getIOSVersion() < 13 && sd.para.heatmap && sd.para.heatmap.collect_tags && sd.para.heatmap.collect_tags.div) {
